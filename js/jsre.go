...
func (jsre *JSRE) runScript(path string) otto.Value {
	...
	if err != nil {
		return jsre.MakeCustomError("FailedToRun", fmt.Sprintf("Failed to run %s: %s", path, err.Error()))
	}
...
loop:
	for {
		...
		case timer := <-jsre.timerReady:
			...
			_, inreg := jsre.timers[timer] // when clearInterval is called from within the callback don't reset it
